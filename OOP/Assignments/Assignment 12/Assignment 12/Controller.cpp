...
int Controller::addToAdoptionList(TElem elem, int op)
{
	...
	if (ok == -1) {
		...
		if (op == 1)
		{
			FileManager* filemng = new TextFileManager();
			filemng->writeToFile(adoptionList, "fisierAdoptie.txt");
		}
		else if (op == 2)
		{
			FileManager* filemng = new HTMLFileManager();
			filemng->writeToFile(adoptionList, "htmlFile.html");
		}
	}
	else
		return -1;
