...
public class ProgramWindowController {
    ...
    @FXML
    private TableView semTbl;

    @FXML
    private TableColumn<Map.Entry<Integer, Triple<Integer, List<Integer>,Integer>>, String> semTblC1;

    @FXML
    private TableColumn<Map.Entry<Integer, Triple<Integer, List<Integer>,Integer>>, String> semTblC2;

    @FXML
    private TableColumn<Map.Entry<Integer, Triple<Integer, List<Integer>,Integer>>, String> semTblC3;
    ...
    public void initialize(){
        ...
        semTblC1.setCellValueFactory(p-> new SimpleStringProperty(p.getValue().getKey() + ""));
        semTblC2.setCellValueFactory(p-> new SimpleStringProperty(p.getValue().getValue() + "")); //???????????? TODO
        semTblC3.setCellValueFactory(p-> new SimpleStringProperty(p.getValue().getValue() + ""));
    ...
    private void populateSemaphoreTable(){
        Map<Integer,  Triple<Integer, List<Integer>,Integer>> sem = controller.getSemTable().getContent();
        List<Map.Entry<Integer, Triple<Integer, List<Integer>,Integer>>> semList = new ArrayList<>();

        for(Map.Entry<Integer, Triple<Integer, List<Integer>,Integer>> elem : sem.entrySet()){
            semList.add(elem);
        }
        semTbl.setItems(FXCollections.observableList(semList));
        semTbl.refresh();
