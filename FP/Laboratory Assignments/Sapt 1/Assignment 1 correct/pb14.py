...
def nelement(n):
    ...
    Output: i- the element on the n-th position
    '''
    s=0
    '''
    s- a counter of the numbers we use to determine the element on the n-th position
    '''
...
def ui_nelement():
    x=0
    while x!=-1:
        x=int(input('Give me the position(-1 to end): '))
        if x<0:
            print('There is no value on that position')
        else:
            print(nelement(x))
